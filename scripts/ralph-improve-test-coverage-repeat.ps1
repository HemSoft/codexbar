# ralph-improve-test-coverage-repeat.ps1 — Repeated test coverage improver.
# Version: 1.3.0
...
if ($Help) {
    Write-Host ""
    Write-Host "ralph-improve-test-coverage-repeat.ps1 - Repeated Test Coverage Improver" -ForegroundColor Cyan
    Write-Host "Runs ralph-improve-test-coverage.ps1 N times in autopilot mode."
    ...
    Write-Host "  ralph-improve-test-coverage-repeat"
    Write-Host "  ralph-improve-test-coverage-repeat -Times 5"
    Write-Host "  ralph-improve-test-coverage-repeat -Times 10 -Model sonnet -WorkUntil 08:00"
    Write-Host "  ralph-improve-test-coverage-repeat -Times 3 -Agents pr-review-quality"
    Write-Host ""
    exit 0
}

$scriptDir = $PSScriptRoot
$targetScript = Join-Path $scriptDir "ralph-improve-test-coverage.ps1"
if (-not (Test-Path $targetScript)) {
    Write-Host "ERROR: ralph-improve-test-coverage.ps1 not found at: $targetScript" -ForegroundColor Red
    exit 1
}
...
for ($i = 1; $i -le $Times; $i++) {
    ...
    Write-Host "== Test coverage improvement run $i of $Times" -ForegroundColor Cyan
    Write-Host "====================================" -ForegroundColor Cyan
...
Write-Host "== All $Times test coverage improvement runs complete!" -ForegroundColor Green
Write-Host "====================================" -ForegroundColor Green
